  ...
  return (
    <div className="border p-4 flex flex-col gap-4" style={{ borderColor }}>
      <div className="flex items-center justify-between">
        <div className="text-[10px] font-bold tracking-[0.2em] uppercase" style={{ color: textColor }}>COLOUR PICKER</div>
        <button 
          className={`p-1.5 rounded-sm transition-all ${isPickerActive ? 'opacity-100 text-blue-500' : 'opacity-40 hover:opacity-100'}`}
          onClick={onPickColor}
          title="Pick color from canvas"
          style={{ 
            backgroundColor: isPickerActive ? (darkMode ? '#1A1A1A' : '#F5F5F5') : 'transparent'
          }}
          onMouseEnter={(e) => {
            if (!isPickerActive) e.currentTarget.style.backgroundColor = darkMode ? '#1F1F1F' : '#F5F5F5';
          }}
          onMouseLeave={(e) => {
            if (!isPickerActive) e.currentTarget.style.backgroundColor = 'transparent';
          }}
        >
          <Pipette className="w-3.5 h-3.5" style={{ color: textColor }} />
        </button>
      </div>
      ...
      <div className="border" style={{ borderColor }}>
        ...
        >
          {/* Square Indicator */}
          <div 
            className="absolute w-2.5 h-2.5 border-[1.5px] shadow-sm pointer-events-none" 
            style={{ 
              top: `${(1 - hsv.v) * 100}%`, 
              left: `${hsv.s * 100}%`, 
      ...
      <div className="flex gap-2 h-8">
        <div 
          className="w-[72px] shrink-0 border flex items-center justify-between px-2 text-[10px] font-bold uppercase cursor-pointer select-none" 
        ...
        {format === 'RGB' ? (
          <>
            <input 
              ...
              className="flex-1 border bg-transparent text-center text-[10px] font-bold focus:outline-none" 
              style={{ borderColor, color: textColor }} 
            />
            <input 
              type="number" 
      ...
      <Collapsible defaultOpen={true}>
        <CollapsibleTrigger className="flex items-center justify-between w-full pt-2">
          <div className="text-[10px] font-bold tracking-[0.2em] uppercase" style={{ color: textColor }}>RECENT SWATCHES</div>
          <ChevronRight className="w-3 h-3 opacity-40" style={{ color: textColor }} />
        </CollapsibleTrigger>
